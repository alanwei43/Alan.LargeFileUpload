...
(function () {
    ...
    window.SeperateFile = function (file, userOptions) {
        ...
        if (userOptions) {
            ...
            uploadConfig.timeOut = (userOptions.durationPerSend || 100) / 2;
        }
        ...
        var interval = function () {
            ...
            if (lastQueue.end > uploadConfig.totalSize) {
                throw "the end of last queue out of total size of file.";
                return;
            }

            if (lastQueue.status === 'unsend') {
                if (uploadConfig.pause) { return; }
                ...
                reader.onload = function () {
                    ...
                    lastQueue.startSendTime = Date.now();
                    var promise = options.send(lastQueue, uploadConfig.queues);
                    promise.then(function () {
                        lastQueue.status = 'sent';
                        lastQueue.finishSendTime = Date.now();
                        lastQueue.spend = lastQueue.finishSendTime - lastQueue.start;
            ...
            if (lastQueue.status === 'sent') {
                ...
                uploadConfig.queues.push({
                    ...
                    isFirst: false,
                    isLast: isLast,
                    index: lastQueue.index + 1,
                    total: lastQueue.total
                });
                options.sent(uploadConfig.queues.shift(), uploadConfig.queues);
        ...
        var utils = {
            ...
            pauseSeperate: function (value) {
                if (value === undefined) {
                    /*
                    * togglePauseStatus()
                    * toggle pause status
                    */
                    uploadConfig.pause = !uploadConfig.pause;
                } else {
                    /*
                    * togglePauseStatus(true)
                    * change pause status
                    */
                    uploadConfig.pause = !!value;
                }
                interval();
            },
            isPause: function () {
                return !!uploadConfig.pause;
            }
        };
        return utils;
    };
